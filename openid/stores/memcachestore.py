...
class MemCacheOpenIDStore(OpenIDStore):
    ...
    def _assocKey(self, server_url, handle):
        ...
        @rtype: str
        """
        hashed_url = oidutil.toBase64(cryptutil.sha1(server_url) + cryptutil.sha1(handle))
        return self.key_prefix + ASSOCATION_CODE + hashed_url

    def _rootKey(self, server_url):
        return self.key_prefix + 'R' + oidutil.toBase64(cryptutil.sha1(server_url))
    ...
    def _getAssociationRecord(self, server_url, handle):
        ...
        try:
            ...
            # Unfortunately, we've orphaned the other associations
            self.memcache.delete(key)
            return None
        else:
            return rec
